...
普通玩家可以将模组分组管理。队伍变更会立即生效：同一队伍内的实体不会互相攻击、造成伤害或施加负面效果，不同队伍的生物会互相敌对。
...
`/showdown team` 下的队伍命令允许普通玩家使用；其他控制命令需要权限等级 2（默认为管理员）。模组参数会根据当前实体注册表自动补全。
...
创建、删除、添加或移除队伍成员时会自动更新敌对关系；`apply` 可用于手动重新应用配置。至少需要两个包含生物实体的队伍才会开始队伍间战斗。
